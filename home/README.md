...
└── └── linux/         # Linux host home modules (kumquat, k3s-*, etc.)
```
...
3. **hosts**: Host entry modules for Home Manager
   - Each output should reference only one host home module file
   - Host modules are responsible for importing shared stacks (`home/linux/*`) and applying host
     overrides
