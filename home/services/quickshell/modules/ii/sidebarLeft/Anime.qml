    ...
    if (event.modifiers === Qt.NoModifier) {
      if (event.key === Qt.Key_PageUp) {
        ...
        booruResponseListView.contentY = Math.min(booruResponseListView.contentHeight,
                                                  booruResponseListView.contentY + root.pageKeyScrollAmount);
        event.accepted = true;
      }
    }
          ...
          onTextChanged: { // Handle tag suggestions
            ...
            if (tagInputField.text.startsWith(`${root.commandPrefix}mode`)) {
              ...
              root.suggestionList = providerResults.map(provider => {
                return {
                  name: `${tagInputField.text.trim().split(" ").length == 1 ? (root.commandPrefix + "mode ") : ""
                                                                              }
...
${provider.target}`,
            ...
            if (tagInputField.text.startsWith(root.commandPrefix)) {
              ...
              root.suggestionList = root.allCommands.filter(cmd => cmd.name.startsWith(tagInputField.text.substring(
                                                                                         1))).map(cmd => {
                                                                                           return {
