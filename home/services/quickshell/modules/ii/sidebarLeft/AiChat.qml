  ...
  Keys.onPressed: event => {
    ...
    if (event.modifiers === Qt.NoModifier) {
      if (event.key === Qt.Key_PageUp) {
        ...
        messageListView.contentY = Math.min(messageListView.contentHeight - messageListView.height / 2,
  ...
  property var allCommands: [
    ...
    },
    {
      ...
      description: Translation.tr(
                     "Set temperature (randomness) of the model. Values range between 0 to 2 for Gemini, 0 to 1 for other models. Default is 0.5."),
      execute: args => {
        // console.log(args)
        if (args.length == 0 || args[0] == "get") {
          Ai.printTemperature();
...
- Arch lincox icon <img src="${Quickshell.shellPath("assets/icons/arch-symbolic.svg")}" height="
...
`, Ai.interfaceRole);
  ...
  Process {
    ...
    function handleEntry(entry: string) {
      ...
      decodeImageAndAttachProc.exec(["bash", "-c", `[ -f ${imageDecodeFilePath} ] || echo '
${StringUtils.shellSingleQuoteEscape(entry)}' | ${Cliphist.cliphistBinary} decode > '${imageDecodeFilePath
                                     }'`]);
    }
  ...
  ColumnLayout {
      ...
      Rectangle {
        ...
        RowLayout {
          ...
          StatusItem {
            ...
            statusText: ""
            description: Ai.currentModelHasApiKey ? Translation.tr(
          ...
          StatusItem {
            ...
            description: Translation.tr("Total token count\nInput: %1\nOutput: %2").arg(
                           Ai.tokenCount.input).arg(Ai.tokenCount.output)
          }
        }
      }
    ...
    FlowButtonGroup { // Suggestions
      ...
      Repeater {
        ...
        delegate: ApiCommandButton {
          id: commandButton
          colBackground: suggestions.selectedIndex === index ? Appearance.colors.colSecondaryContainerHover :
    ...
    Rectangle { // Input area
      ...
      implicitHeight: Math.max(inputFieldRowLayout.implicitHeight + inputFieldRowLayout.anchors.topMargin
                               ...
                               + spacing, 45) + (attachedFileIndicator.implicitHeight + spacing
                                                 + attachedFileIndicator.anchors.topMargin)
      clip: true
            ...
            onTextChanged: {
              ...
              if (messageInputField.text.length === 0) {
                ...
                root.suggestionList = modelResults.map(model => {
                  return {
                    name: `${messageInputField.text.trim().split(" ").length == 1 ? (root.commandPrefix
                                                                                     + "model ") : ""}
${model.target}`,
...
${tool.target}`,
                ...
                root.suggestionList = root.allCommands.filter(cmd => cmd.name.startsWith(
                                                                       messageInputField.text.substring(
                                                                         1))).map(cmd => {
                                                                           return {
...
${cmd.name}`,
            ...
            Keys.onPressed: event => {
              if (event.key === Qt.Key_Tab) {
                ...
                suggestions.selectedIndex = Math.min(root.suggestionList.length - 1,
      ...
      RowLayout { // Controls
        ...
        ApiInputBoxIndicator {
          ...
          tooltipText: Translation.tr("Current model: %1\nSet it with %2model MODEL").arg(Ai.getModel(
                                                                                            ).name).arg(
                         root.commandPrefix)
        }
