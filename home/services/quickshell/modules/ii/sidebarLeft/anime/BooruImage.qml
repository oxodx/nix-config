...
Button {
  ...
  property string fileName: decodeURIComponent((imageData.file_url).substring((imageData.file_url).lastIndexOf(
                                                                                '/') + 1))
  ...
  contentItem: Item {
    ...
    RippleButton {
      ...
      colBackgroundHover: ColorUtils.transparentize(ColorUtils.mix(Appearance.m3colors.m3surface,
                                                                   Appearance.m3colors.m3onSurface, 0.8), 0.2)
      colRipple: ColorUtils.transparentize(ColorUtils.mix(Appearance.m3colors.m3surface,
                                                          Appearance.m3colors.m3onSurface, 0.6), 0.1)
        ...
        Rectangle {
          ...
          ColumnLayout {
            ...
            MenuButton {
              ...
              Layout.fillWidth: true
              buttonText: Translation.tr("Go to source (%1)").arg(StringUtils.getDomain(
                                                                    root.imageData.source))
            ...
            MenuButton {
              ...
              onClicked: {
                ...
                const userAgentHeader = userAgent ? ` -H 'User-Agent: ${StringUtils.shellSingleQuoteEscape(
                                                      userAgent)}'` : "";
                Quickshell.execDetached(["bash", "-c", `mkdir -p '${targetPath}' && curl '
${StringUtils.shellSingleQuoteEscape(root.imageData.file_url)}'${userAgentHeader} -o '${targetPath}/
${root.fileName}' && notify-send '${Translation.tr("Download complete")}' '${root.downloadPath}/
${root.fileName}' -a 'Shell'`]);
              }
            }
          }
        }
