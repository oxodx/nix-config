...
ButtonMouseArea {
  ...
  readonly property real barThickness: vertical ? Appearance.sizes.verticalBarWidth :
    ...
    WorkspaceLayout {
      ...
      Repeater {
        ...
        delegate: Item {
          ...
          property bool previousOccupied: index > 0 && wsModel.occupied[index - 1] && (wsId - 1)
          != wsModel.fakeWorkspace
          property bool nextOccupied: index < wsModel.shownCount - 1 && wsModel.occupied[index + 1] && (wsId
                                                                                                        + 1) != wsModel.fakeWorkspace
          ...
          Pill {
            ...
            property real undirectionalLength: root.workspaceButtonWidth * (1 + 0.5 * wsBg.previousOccupied
                                                                            + 0.5 * wsBg.nextOccupied)
                                               * currentOccupied
            property real undirectionalOffset: (!wsBg.currentOccupied ? 0.5 : -0.5 * wsBg.previousOccupied)
    ...
    WorkspaceLayout {
      ...
      Repeater {
        ...
        delegate: WorkspaceItem {
          ...
          property var mainAppIconSource: Quickshell.iconPath(AppSearch.guessIcon(biggestWindow?.class),
                                                              "image-missing")

          AppIcon {
            id: appIcon
            property real cornerMargin: (!root.superPressAndHeld && Config.options
                                         ?.bar.workspaces.showAppIcons && wsApp.biggestWindow) ? (
                                                                                                   root.workspaceButtonWidth
                                                                                                   - root.workspaceIconSize)
                                                                                                 / 2 : root.workspaceIconMarginShrinked
            anchors {
          ...
          Loader { // Somehow putting this multieffect in a loader prevents it from not showing up
            ...
            sourceComponent: Colorizer {
              ...
              colorizationColor: Appearance.m3colors.darkmode ? Appearance.colors.colOnSecondaryContainer :
                                                                Appearance.colors.colOnPrimary
              colorization: Config.options.bar.workspaces.monochromeIcons ? 0.8 : 0.5
              brightness: 0
              source: appIcon

              opacity: !Config.options?.bar.workspaces.showAppIcons ? 0 : (wsApp.biggestWindow &&
                                                                           !root.superPressAndHeld
                                                                           && Config.options
                                                                           ?.bar.workspaces.showAppIcons) ? 1 :
                                                                                                            wsApp.biggestWindow
                                                                                                            ? root.workspaceIconOpacityShrinked :
                                                                                                              0
              visible: opacity > 0
              scale: ((!root.superPressAndHeld && Config.options?.bar.workspaces.showAppIcons)
                      ? root.workspaceIconSize : root.workspaceIconSizeShrinked) / root.workspaceIconSize
  ...
  FadeLoader {
    ...
    sourceComponent: Pill {
      ...
      property real undirectionalLength: {
        const base = root.workspaceButtonWidth * Math.min(1.35,
                                                          wsModel.shownCount); // Who tf only configures only 2 workspaces shown anyway?
        if (root.vertical)
        return base;
  ...
  component NumberWorkspaceItem: WorkspaceItem {
    ...
    property color contentColor: (wsModel.occupied[wsNum.index] && wsId !== wsModel.fakeWorkspace)
    ...
    property bool showingNumbers: {
      ...
      return false;
      if (Config.options?.bar.workspaces.alwaysShowNumbers && (!Config.options?.bar.workspaces.showAppIcons ||
                                                               !wsNum.hasBiggestWindow))
      return true;
      return false;
    }
  ...
  component TrailingIndicator: Item {
    ...
    StyledRectangle {
      ...
      property real indicatorPosition: Math.min(idxPair.idx1, idxPair.idx2) * root.workspaceButtonWidth
                                       + root.activeWorkspaceMargin
      property real indicatorLength: Math.abs(idxPair.idx1 - idxPair.idx2) * root.workspaceButtonWidth
                                     + root.activeWorkspaceSize
