...
Column {
  ...
  property var keySubstitutions: Object.assign({
                                                 ...
                                                 // "Shift": "",
                                                                                         ...
                                                                                         } : {}, Config.options.cheatsheet.useMacSymbol
                                               ? macSymbolMap : {}, Config.options.cheatsheet.useFnSymbol
                                               ? functionSymbolMap : {},
    ...
    Repeater {
      ...
      model: {
        if (!root.isCategorized) {
          return HyprlandKeybinds.keybinds.filter(bind => root.hasDescription(bind) && root.isUncategorized(
                                                            bind) && !root.containsNonFirstRepetitive(bind));
        }
        return HyprlandKeybinds.keybinds.filter(bind => root.hasDescription(bind) && root.isCategory(bind,
                                                                                                     root.categoryName)
                                                        && !root.containsNonFirstRepetitive(bind));
      }
      delegate: BindLine {
  ...
  component BindLine: Row {
    ...
    Row {
      ...
      Row {
        ...
        Repeater {
          model: {
            const modList = root.modMaskToStringList(bindLine.keyData.modmask).map(mod
                                                                                   => root.keySubstitutions[mod]
                                                                                      || mod);
            if (modList.length == 0)
            return [];
