...
AbstractOverlayWidget {
  ...
  cursorShape: {
    ...
    if (resizeMargin < mouseX && mouseX < width - resizeMargin && resizeMargin < mouseY && mouseY < height
        - resizeMargin) {
  ...
  onPressed: event => {
    ...
    if (!root.resizable)
      return;
    if (root.resizeMargin < event.x && event.x < root.width - root.resizeMargin && root.resizeMargin
  ...
  onPositionChanged: event => {
    if (!resizing)
      return;
    contentContainer.implicitWidth = Math.max(root.persistentStateEntry.width + dragHandler.xAxis.activeValue
                                              * root.resizeXDirection, root.minimumWidth);
    contentContainer.implicitHeight = Math.max(root.persistentStateEntry.height
                                               + dragHandler.yAxis.activeValue * root.resizeYDirection,
                                               root.minimumHeight);
    ...
    const negativeXDragLimit = root.persistentStateEntry.x + root.persistentStateEntry.width
          - contentContainer.implicitWidth;
    const negativeYDragLimit = root.persistentStateEntry.y + root.persistentStateEntry.height
  ...
  function savePosition(xPos = root.x, yPos = root.y, width = contentContainer.implicitWidth, height
                        = contentContainer.implicitHeight) {
  ...
  function center() {
    ...
    const targetY = (root.parent.height - contentContainer.height) / 2 - titleBar.implicitHeight
          + border.border.width - root.resizeMargin;
    root.x = targetX;
    root.y = targetY;
  ...
  Rectangle {
    ...
    color: ColorUtils.transparentize(Appearance.colors.colLayer1Base, (root.fancyBorders
                                                                       && GlobalStates.overlayOpen) ? 0 : 1)
    radius: root.radius
    border.color: ColorUtils.transparentize(Appearance.colors.colOutlineVariant, GlobalStates.overlayOpen ? 0 :
                                                                                                            1)
    border.width: 1
    ...
    ColumnLayout {
      ...
      Item {
        ...
        Layout.topMargin:
          -border.border.width // Border of a rectangle is drawn inside its bounds, so we do this to make the gap not too big
  ...
  component TitlebarButton: RippleButton {
    ...
    contentItem: Item {
      ...
      MaterialSymbol {
        ...
        color: titlebarButton.toggled ? Appearance.colors.colOnSecondaryContainer :
