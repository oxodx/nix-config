...
OverlayBackground {
  ...
  function applyLimit() {
    ...
    var updateCommands = cfgPaths.map(path => {
      return "if grep -q '^fps_limit=' " + path + "; " + "then sed -i 's/^fps_limit=.*/fps_limit=" + fpsValue
          + "/' " + path + "; " + "else echo 'fps_limit=" + fpsValue + "' >> " + path + "; fi";
    }).join("; ");
  ...
  RowLayout {
    ...
    ToolbarTextField {
      ...
      placeholderText: root.currentState === FpsLimiterContent.State.Error ? Translation.tr(
                                                                               "Enter a valid number") :
                                                                             Translation.tr("Set FPS limit")
