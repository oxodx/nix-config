...
Rectangle {
  ...
  Popup {
    ...
    property point originPoint: categoryOpenButtonLoader.mapToItem(root, categoryOpenButtonLoader.width / 2,
                                                                   categoryOpenButtonLoader.height / 2)
    property point windowCenterPoint: {
      const rootContentItem = root.windowRootItem;
      const canvasPosInRoot = root.mapFromItem(rootContentItem, rootContentItem.width / 2,
                                               rootContentItem.height / 2);
      const sectionItem = root.parent.parent.parent;
      const positionInSection = sectionItem.mapFromItem(categoryOpenButtonLoader, categoryOpenButtonLoader.x,
                                                        categoryOpenButtonLoader.y);
            ...
            Repeater {
              ...
              delegate: Item {
                ...
                BigAppGrid {
                  ...
                  columns: 4
                  rows: 3
                  desktopEntries: root.desktopEntries.slice(folderPage.index * 12, (folderPage.index + 1)
                                                            * 12)
  ...
  component SmallGridAppButton: SmallGridButton {
    ...
    WMenu {
      ...
      WMenuItem {
        ...
        text: smallGridAppButton.pinnedStart ? Translation.tr("Unpin from Start") : Translation.tr(
                                                 "Pin to Start")
        onTriggered: {
      ...
      WMenuItem {
        ...
        text: smallGridAppButton.pinnedTaskbar ? Translation.tr("Unpin from taskbar") : Translation.tr(
                                                   "Pin to taskbar")
        onTriggered: {
