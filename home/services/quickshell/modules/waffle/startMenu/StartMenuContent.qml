  ...
  Keys.onPressed: event => {
    ...
    if (event.key === Qt.Key_Backspace) {
      ...
      if (event.modifiers & Qt.ControlModifier) {
        ...
        if (searchBar.searchInput.cursorPosition > 0) {
          searchBar.text = searchBar.text.slice(0, searchBar.searchInput.cursorPosition - 1) + searchBar.text.slice(
        ...
        >= 0x20) // ignore control chars like Backspace, Tab, etc.
    {
      if (!searchBar.searchInput.activeFocus) {
        ...
        searchBar.text = searchBar.text.slice(0, searchBar.searchInput.cursorPosition) + event.text
            + searchBar.text.slice(searchBar.searchInput.cursorPosition);
        searchBar.searchInput.cursorPosition += 1;
        event.accepted = true;
