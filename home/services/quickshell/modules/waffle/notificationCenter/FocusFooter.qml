...
FooterRectangle {
  ...
  RowLayout {
    ...
    WTextWithFixedWidth {
      ...
      text: Translation.tr("%1 mins").arg(`<font color="${Looks.colors.fg.toString()}">
${TimerService.focusTime / 60}</font>`)
    }
    ...
    SmallBorderedIconAndTextButton {
      ...
      onClicked: {
        if (TimerService.pomodoroRunning) {
          ...
          TimerService.togglePomodoro();
          Quickshell.execDetached(["qs", "-p", Quickshell.shellPath(""), "ipc", "call", "sidebarRight",
                                   "toggle"]);
        }
      }
    }
