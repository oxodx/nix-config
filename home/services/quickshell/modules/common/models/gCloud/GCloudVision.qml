...
GCloudApi {
  ...
  function annotateImage(imageUri: string) {
    ...
    seq = [ //
           ["bash", "-c", `mkdir -p '${Directories.screenshotTemp}'; base64 '${niceFilePath}' -w 0 > '
${imageBase64FilePath}'`] //
           , out => { //
  ...
  onPreparationReadyChanged: {
    ...
    seq.push(["bash", "-c", `jq -n --rawfile content '${imageBase64FilePath}' \
'{"requests": [{"image": {"content": $content}, "features": [{"type": "DOCUMENT_TEXT_DETECTION"}]}]}' \
> '${payloadFilePath}'`]);

    seq.push(["bash", "-c", `curl -s -X POST \
