...
TabButton {
  ...
  property color colBackgroundHover: ColorUtils.transparentize(Appearance.colors.colOnSurface, root.checked
                                                               ? 1 : 0.95)
  ...
  MouseArea {
    ...
    onPressed: event => {
      ...
      rippleAnim.radius = Math.sqrt(Math.max(dist(0, stateY), dist(0, stateEndY), dist(width, stateY), dist(
                                               width, stateEndY)));
