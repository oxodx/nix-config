...
MouseArea { // Notification group area
  ...
  Rectangle { // Background of the notification
    ...
    clip: true
    implicitHeight: root.expanded ? row.implicitHeight + padding * 2 : Math.min(80, row.implicitHeight
                                                                                + padding * 2)
    ...
    RowLayout { // Left column for icon, right column for content
      ...
      ColumnLayout { // Content
        ...
        spacing: expanded ? (root.multipleNotifications ? (notificationGroup
                                                           ?.notifications[root.notificationCount - 1].image
                                                           != "") ? 35 : 5 : 0) : 0
        // spacing: 00
        Behavior on spacing {
          animation: Appearance.animation.elementMoveFast.numberAnimation.createObject(this)
        ...
        Item { // App name (or summary when there's only 1 notif) and time
          ...
          RowLayout {
            ...
            StyledText {
              ...
              Layout.fillWidth: true
              text: (topRow.showAppName ? notificationGroup?.appName : notificationGroup?.notifications[0]
                                          ?.summary) || ""
        ...
        StyledListView { // Notification body (expanded)
          ...
          model: ScriptModel {
            values: root.expanded ? root.notifications.slice().reverse() : root.notifications.slice().reverse(
                                      ).slice(0, 2)
          }
