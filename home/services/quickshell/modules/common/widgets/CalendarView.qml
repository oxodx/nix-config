...
Item {
  ...
  function scrollMonthsAndSnap(x) { // Scroll x months and snap to month
    ...
    const currentFirstShownDate = new Date(root.dateInFirstWeek.getTime() + (root.paddingWeeks
                                                                             * root.millisPerWeek));
  ...
  MouseArea {
    anchors.fill: parent
    onWheel: wheel => {
      root.targetWeekDiff += wheel.angleDelta.y / 120 *
          -root.weeksPerScroll; // Reverse cuz scrolling down should advance
    }
  }
  ...
  property date dateInFirstWeek: {
    ...
    return new Date(firstDayThisMonth.getTime() - (paddingWeeks * millisPerWeek) + contentWeekDiff
                    * millisPerWeek);
  ...
  property date focusedDate: {
    ...
    return DateUtils.getIthDayDateOfSameWeek(dateInTargetWeek, root.focusDayOfWeekIndex
                                             - root.locale.firstDayOfWeek,
                                             root.locale.firstdayOfWeek); // 4 = Thursday
  }
