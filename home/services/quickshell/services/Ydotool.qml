...
Singleton {
  ...
  function releaseAllKeys() {
    ...
    Quickshell.execDetached(["ydotool", "key", "--key-delay", "0", ...keycodes.map(keycode => `${keycode
                                                                                              }:0`)]);
    root.shiftMode = 0; // Reset shift mode
  }

  function releaseShiftKeys() {
    Quickshell.execDetached(["ydotool", "key", "--key-delay", "0", ...root.shiftKeys.map(keycode => `${keycode
                                                                                                    }:0`)]);
    root.shiftMode = 0; // Reset shift mode
  }
