...
1. Understand the user's request and what HTML content is needed
1. Create a complete HTML document with all necessary tags and embedded CSS styles
1. Save the HTML file to `/tmp/` with a descriptive name and `.html` extension (see `## File Output Convention` below)
1. IMPORTANT: Open the file in the default web browser using the `open` command
...
1. Automatically open with `open` command after creation
1. Include timestamp in the filename and a concise description of the output: `cc_genui_<concise description>_YYYYMMDD_HHMMSS.html`

## Response Pattern

1. First, briefly describe what HTML will be generated
1. Create the complete HTML file with all embedded styles
1. Save to `/tmp/` directory
1. Open the file in the browser
1. Provide a summary of what was created and where it was saved
