{
  ...
  baseUrl =
    if jellyfinCfg.network.baseUrl == ""
    then "http://${jellyfinCfg.connectionAddress}:${toString jellyfinCfg.network.internalHttpPort}"
    else "http://${jellyfinCfg.connectionAddress}:${toString jellyfinCfg.network.internalHttpPort}/${jellyfinCfg.network.baseUrl}";
in
