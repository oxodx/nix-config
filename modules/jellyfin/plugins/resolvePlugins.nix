{
  ...
  highestByTargetAbi = matches:
    if matches == []
    then []
    else let
      highestMatch = lib.foldl' (
        best: candidate:
          if lib.versionOlder (normalizeTargetAbi best.targetAbi) (normalizeTargetAbi candidate.targetAbi)
          then candidate
          else best
            ...
            builtins.readFile (
              builtins.fetchurl {
                ...
                in
                  if isSha256SRI hash
                  then
                    if builtins.convertHash or null == null
                    then throw "Hash is in SRI format, but builtins.convertHash is not available in this version of Nix. Please provide the hash in nix32 format instead."
                    else
  ...
  findPluginSource = pluginName: sourceSpec: let
    pluginVersion = sourceSpec.version;
    repositoryName = sourceSpec.repository or null;
    matchingRepositories =
      if repositoryName == null
      then repositoriesWithManifest
      else lib.filter (repo: repo.name == repositoryName) repositoriesWithManifest;
    ...
    compatibleAbi =
      lib.filter (
        ...
        in
          versionSeries normalizedTargetAbi
          == versionSeries jellyfinVersion
          && !lib.versionOlder jellyfinVersion normalizedTargetAbi
      )
      versionMatches;

    selectedMatches =
      if lib.length matchingAbi == 1
      then matchingAbi
      else versionMatches;

    selectedCompatibleMatches =
      if lib.length matchingAbi > 0
      then matchingAbi
      else highestByTargetAbi compatibleAbi;

    matchingRepositoriesSummary = lib.concatStringsSep ", " (
      lib.unique (map (match: match.name) selectedCompatibleMatches)
    );
  in
    if repositoryName != null && matchingRepositories == []
    then throw "nixflix.jellyfin.plugins.\"${pluginName}\": repository '${repositoryName}' was not found in nixflix.jellyfin.system.pluginRepositories"
    else if versionMatches == []
    then throw "nixflix.jellyfin.plugins.\"${pluginName}\": version '${pluginVersion}' was not found in any configured plugin repository"
    ...
    then throw "nixflix.jellyfin.plugins.\"${pluginName}\": version '${pluginVersion}' matched multiple releases. Add `repository` or update the resolver to disambiguate the target ABI for Jellyfin ${jellyfinVersion}."
    else {match = lib.head selectedMatches;};

  packagePluginDirName = plugin: let
    passthru = plugin.passthru or {};
    version = lib.getVersion plugin;
    name = lib.getName plugin;
  in
    passthru.pluginDirName or (
      if version == ""
      then name
      else "${name}_${version}"
    );

  resolvePluginResult = pluginName: pluginCfg:
    if pluginCfg.package == null || lib.isDerivation pluginCfg.package
    then {
      inherit pluginCfg;
    }
    else let
