...
with lib; let
  ...
  countDefaults = mode: builtins.length (builtins.filter (t: t.mode == mode && t.isDefault) cfg.overlays.templates);
in {
  imports = [./options.nix];
